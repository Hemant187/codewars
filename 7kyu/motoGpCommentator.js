...
//P: str, number valid
//R: name starts from the position of the row
//E: 'Valentino Rossi', 11
// Example Output: 'Valentino Rossi starts from the middle of the 4th row
//PseudoCode: pos =1 'name pole', count row(){row = math.ceil(position-1 / 3), [front,2nd,3rd] || row'th'}, count position(array=[top, bottom, middle]) array[postion%3] 

function moto(name, position){
  if(position === 1) return `${name} starts from pole position`
  const row  = Math.ceil(position/3) -1
  const pos = ['top','middle','bottom'][(position - 1) %3]
  const rowStr = ['front', '2nd', '3rd'][row] || ((row + 1) + 'th')
  return `${name} starts from the ${pos} of the ${rowStr} row`
}