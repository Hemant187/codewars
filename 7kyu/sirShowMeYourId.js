...
//Params: string
//Result: true, false
//Example: 'Jean-Michel' -> true, 'Jean-' -> false, 'jean' -> false, "Jean" => true
//PseudoCode: if (' ') -> false; regex = A-Z+a-z+... split('-')
//let's code:
function showMeYourId(name){
  if(name.includes(' ')) return false;
  const regex = /^[A-Z][a-z]+$/
  return console.log(name.split('-').every(match => regex.test(match)))
}

showMeYourId('Francis-')