...
function spot(s1,s2){
  //P: Campare strings, return list of character index that are different.
  //R: list, string length same, case senstive characters
  //E: ('abcdefg', 'abcqetg') -> [3,5]
  //PseudoCode: define an array, iterate through str and check if characters are dfferent add index to array
  const result = []
  for(let i = 0; i<s1.length; i++){
    s1[i] != s2[i] && result.push(i)
  }
  return result;
}